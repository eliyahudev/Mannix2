...
//    v1.4     J.Stoickov        08 Aug 07   WEL bit corrected
//    V1.5     J.Stoickov        08 Dec 10   Latest datasheet aligned
//                                           (S25FL129 064 032P)
//                                           Program Error bit will not be
//                                           set after programming in
//                                           protect memory region
//    V1.6     V.Mancev          10 Sep 29   Implementation of internal pull-up
//                                           for HOLDNeg and WPNeg pins
//    V1.7     V.Mancev          12 Jan 13   Latest datasheet aligned
//                                           (S25FL064P_00_05)
//////////////////////////////////////////////////////////////////////////////
//  PART DESCRIPTION:
//
...
//  internal delays
    ...
    parameter mem_file_name     = "none";//"s25fl064p.mem";
    parameter otp_file_name     = "none";//"s25fl064p_secsi.mem";

    parameter TimingModel   = "DefaultTimingModel";

    parameter PartID        = "s25fl064p";
    parameter MaxData       = 255;
    parameter SecSize       = 16'hFFFF;
    parameter SecSize_4     = 4095;
    parameter SecSize_8     = 8191;
    parameter SecNum        = 127;
    parameter PageNum       = 16'h7FFF;
    ...
    parameter DeviceID      = 16'h0216;
    parameter ExtendedBytes = 8'h4D;
    parameter ReservedBytes = 8'h00;
    ...
    reg  hold_mode;
    reg  mpm_mode = 0;
    reg  initial_config = 0;
    wire hold;

    //Sector Protection Status
    reg [SecNum:0] Sec_Prot = 127'b0; //= SecNum'b0;
...
///////////////////////////////////////////////////////////////////////////////
 ...
 specify
     ...
     specparam           tperiod_SCK_rdid            =1; // fSCK = 50MHz
        ...
        `ifdef SPEEDSIM
            ...
            specparam   tdevice_BE                     = 128e9; //128 ms;
            ...
            specparam   tdevice_PE                     = 800e6; //800 us;
                    //VCC (min) to CS# Low
            specparam   tdevice_PU                     = 3000000 ; //Roy changed to 3us was 300000000; //300 us;
        `else
            // Page Program Operation
            ...
            specparam   tdevice_SE                     = 2e12; //2 s;
                    //Bulk Erase Operation
            specparam   tdevice_BE                     = 128e12; //128 sec;
                    //Write Status Register Operation
            ...
            specparam   tdevice_PE                     = 800e9; //800 ms;
                    //VCC (min) to CS# Low
            specparam   tdevice_PU                     = 300000000; //300 us;
        `endif//SPEEDSIM
...
////////////////////////////////////////////////////////////////////////////////
        ...
        $width (posedge SCK &&& rd_jid  , tpw_SCK_rdid_posedge);
        ...
        $width (negedge SCK &&& rd_jid  , tpw_SCK_rdid_negedge);
        ...
        $period (posedge SCK &&& rd_jid,  tperiod_SCK_rdid);
...
//Bus cycle states
 ...
 reg deq_holdin;
    ...
    integer j;
            ...
            CFI_array[8'h45] = 8'h15;
            CFI_array[8'h46] = 8'h00;
            CFI_array[8'h47] = 8'h02;
    ...
    integer i;
        ...
        begin
           // Memory Preload
           //s25fl064p.mem, memory preload file
        ...
        begin
            ...
            begin
                ...
                begin
                    bus_cycle_state = STAND_BY;
                    if (((HOLDNeg_in && ~QUAD) || QUAD) &&
                       (Instruct == RES_READ_ES) && (dummy_cnt == 0))
                        write = 1'b0;
                end
            end
        end
...
//    /////////////////////////////////////////////////////////////////////////
            ...
            case (current_state)
                ...
                begin
                    ...
                    begin
                        if ((Instruct == WRR) && WEL)
                        begin
                            if (~(Status_reg[7] && ~WPNeg_in && ~QUAD))
                            //HPM is disabled when the Quad I/O Mode is enabled
                            //(Quad bit = 1 in the Configuration Register)
                                next_state = WRITE_SR;
                        end
                        ...
                        begin
                            ...
                            if (Sec_Prot[sect] == 1'b0 && (((sect == 0 ||
                                sect == 1) && ~TBPARM) || ((sect == SecNum ||
                                sect == SecNum-1) && TBPARM)))
                                next_state = P4_ER;
                        end
    ...
    integer i,j;
        ...
        begin
            ...
            //The WEL bit resets (disables writes) on power-up
            Status_reg[1] = 1'b0;

            if (BPNV == 1)
            begin
            //When BPNV is set to ‘1’,the BP2-0 bits in the Status Register
            //are volatile and will be reset to binary 111 after power on reset.
                Status_reg[4] = 1'b1;// BP2
                Status_reg[3] = 1'b1;// BP1
                Status_reg[2] = 1'b1;// BP0
                change_BP = 1'b1;
                #1 change_BP = 1'b0;
            end
        end
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    //SRWD bit
                    Status_reg[7] = Status_reg_in[7];//MSB first
                    ...
                    //including FREEZE are writeable.
                        ...
                        if (cfg_write)
                        begin
                            Sec_conf_reg[0] = Sec_conf_reg_in[0];// FREEZE

                            if (TBPARM == 1'b0 && initial_config == 1'b0)
                            begin
                                Sec_conf_reg[2] = Sec_conf_reg_in[2];//TBPARM
                            end

                            if (TBPROT == 1'b0 && initial_config == 1'b0)
                            begin
                                Sec_conf_reg[5] = Sec_conf_reg_in[5];//TBPROT
                            end
                        end
                    ...
                    if (cfg_write)
                    begin
                        Sec_conf_reg[1] = Sec_conf_reg_in[1];// QUAD

                        if (BPNV == 1'b0)
                        begin
                            Sec_conf_reg[3] = Sec_conf_reg_in[3];//BPNV
                        end
                    end
        ...
        begin
            case (Status_reg[4:2])
                ...
                begin
                    if (~Sec_conf_reg[5])
                    begin
                        Sec_Prot[SecNum : (SecNum+1)*63/64] = 2'b11;
                        Sec_Prot[(SecNum+1)*63/64 - 1 : 0] = 126'h0;
                    end
                    else
                    begin
                        Sec_Prot[(SecNum+1)/64 - 1 : 0] = 2'b11;
                        Sec_Prot[SecNum : (SecNum+1)/64] = 126'h0;
                    end
                end
                ...
                begin
                    if (~Sec_conf_reg[5])
                    begin
                        Sec_Prot[SecNum : (SecNum+1)*31/32] = 4'hF;
                        Sec_Prot[(SecNum+1)*31/32 - 1 : 0] = 124'h0;
                    end
                    else
                    begin
                        Sec_Prot[(SecNum+1)/32 - 1 : 0] = 4'hF;
                        Sec_Prot[SecNum : (SecNum+1)/32] = 124'h0;
                    end
                end
                ...
                begin
                    if (~Sec_conf_reg[5])
                    begin
                        Sec_Prot[SecNum : (SecNum+1)*15/16] = 8'hFF;
                        Sec_Prot[(SecNum+1)*15/16 - 1 : 0]  = 120'h0;
                    end
                    else
                    begin
                        Sec_Prot[(SecNum+1)/16 - 1 : 0]  = 8'hFF;
                        Sec_Prot[SecNum : (SecNum+1)/16] = 120'h0;
                    end
                end
                ...
                begin
                    if (~Sec_conf_reg[5])
                    begin
                        Sec_Prot[SecNum : (SecNum+1)*7/8] = 16'hFFFF;
                        Sec_Prot[(SecNum+1)*7/8 - 1 : 0]  = 112'h0;
                    end
                    else
                    begin
                        Sec_Prot[(SecNum+1)/8 - 1 : 0]  = 16'hFFFF;
                        Sec_Prot[SecNum : (SecNum+1)/8] = 112'h0;
                    end
                end
                ...
                begin
                    ...
                    begin
                        Sec_Prot[SecNum : (SecNum+1)*3/4]
                                                    = 32'hFFFFFFFF;
                        Sec_Prot[(SecNum+1)*3/4 - 1 : 0] = 96'h0;
                    end
                    else
                    begin
                        Sec_Prot[(SecNum+1)/4 - 1 : 0]
                                                    = 32'hFFFFFFFF;
                        Sec_Prot[SecNum : (SecNum+1)/4] = 96'h0;
                    end
                end
