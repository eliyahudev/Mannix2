...
void uart_set_cfg(int parity, uint16_t clk_counter) {
  ...
  *(volatile unsigned int*)(UART_REG_SCR) = ((*(volatile unsigned int*)(UART_REG_SCR)) & 0xFE) ; // Disable enable_hashtag (enabled by default) 
...
void uart_sendchar(const unsigned char c) {
  // wait until there is space in the fifo
  while( (*(volatile unsigned int*)(UART_REG_LSR) & 0x20) == 0);
