...
int compare4DTensors( Tensor4D_uint8* t1, Tensor4D_uint8* t2 ) {
	int valid = 1;
	for (int i = 0; i < t1->dim; i++){
		if (!compareTensors(&t1->tensor[i],&t2->tensor[i])) valid = 0;
	}
	return valid;
}
...
int setImage(Tensor4D_uint8* tens_4d, FILE_PTR fd) {

    unsigned int label=-1;// TODO - well define NULL 
...
void print4DTensor_uint8(Tensor4D_uint8* tens_4d) {
    ...
    bm_printf("}\n");
    bm_printf("rows size: %d columns size: %d depth size: %d dim: %d \n\n",tens_4d->rows,tens_4d->cols,tens_4d->depth,tens_4d->dim);
...
Tensor4D_uint8* tensor4DMaxPool(Tensor4D_uint8* tens_4d, Tensor4D_uint8* tens_4d_result, int p_m, int p_n, int stride, Allocator_uint8* al, MatAllocator_uint8* mat_alloc, TensorAllocator_uint8* tens_alloc, int work_mode) {
    ...
    for (int i = 0; i < tens_4d->dim; i++) {
        tensorMaxPool_uint8(&tens_4d->tensor[i], &tens_4d_result->tensor[i], p_m, p_n, stride, work_mode);
    }
