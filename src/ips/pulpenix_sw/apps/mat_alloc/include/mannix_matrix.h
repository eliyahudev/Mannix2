...
void setMatrixValues_int32(Matrix_int32* m1, FILE_PTR filePointer, unsigned int* label, int op) {
...
void setMatrixValues_int8(Matrix_int8* m1, FILE_PTR filePointer, unsigned int* label, int op) {
    #ifndef MEM_LOAD_MODE
...
void setMatrixValues_uint8(Matrix_uint8* m1, FILE_PTR filePointer, unsigned int* label, int op) {
    #ifndef MEM_LOAD_MODE
    getData_uint8(filePointer, m1[0].rows * m1[0].cols  + 1 , label, m1[0].data, op);
    #else
      unsigned int matrix_idx ; // For future tracking , currently not used
...
void printMatrix_uint8(Matrix_uint8* m1) {
    if(m1->rows <= 0 || m1->cols <= 0) {
        bm_printf("Dimension ERRER - non positive hight or width  [%d][%d]\n",m1-> cols, m1->rows);
        exit(-1);
    }
    int i=0;
    while(i<m1[0].rows * m1[0].cols) {
...
int getMatrix_uint8(Matrix_uint8* m1, FILE_PTR filePointer, unsigned int* label, int state, int op){
	#ifndef BARE_METAL			  
	if(feof(filePointer)) return -1;
	#endif
	// set matrix data
...
int getMatrix_int8(Matrix_int8* m1, FILE_PTR filePointer, unsigned int* label, int state, int op){
    #ifndef BARE_METAL					  
    if(feof(filePointer)) return -1;
    #endif
    // set matrix data
...
int getMatrix_int32(Matrix_int32* m1, FILE_PTR filePointer, unsigned int* label, int state, int op){
    #ifndef BARE_METAL
    if(feof(filePointer)) return -1;
    #endif
    // set matrix data
...
void writeMatrixToCsv_uint8 (Matrix_uint8* m1, char* layer_name) {
    printf(layer_name);
    printf("\n");
    FILE_PTR fptr = fopen(/*"../test_products/test.csv"*/layer_name, "w");
    if(fptr != NULL) 
...
int dot_prod_i8Xui8(signed char * v1, unsigned char* v2, int len) {
  int result_val = 0 ;
  for (int i=0;i<len;i++) result_val += v1[i]*v2[i]  ;   
  return result_val ;
}
   ...
   if (m1->cols != m2->rows ) {
    ...
    for (int i=0; i< m1->rows; i++) {
        for (int j=0; j < m2->cols; j++) {
			if (m2->cols==1) result_matrix->data[i] = dot_prod_i8Xui8((signed char *)&(m1->data[i*m1->cols]),(unsigned char *)m2->data,m1->cols) ; //  continues vectors in memory
            else for (int k=0; k < m1->cols; k++) {
                if (k == 0) {
...
void setBias(Matrix_int32* bias, char* file_path, char* type, int layer, char* w_b) {
    ...
    FILE_PTR fd = createFdFc(file_path, type, layer, w_b);
...
#if (defined ONLY_KERNEL_5X5_USED) && (defined PULP_EXT) && (defined BARE_METAL)
#define CONV_5X5_OPT_AVAIL
#endif

//#ifndef CONV_5X5_OPT_AVAIL // not required in case of CONV_5X5_OPT_AVAIL
...
// p_m - window's hight
// p_n - window's width
Matrix_uint8* ACCELERATOR_matrixMaxPool(Matrix_uint8* m1, Matrix_uint8* m2, int p_m, int p_n, int stride){
...
// Warning missing while loop for convnlution 
// TODO ask Udi if he meant to replace hadamard mull 
#else
 //             In_Img,   Out_Img,             R,         C,        Kernel,         bias 
    Conv5x5_Vector(m1->data, result_matrix->data, m1->rows,  m1->cols, m_filter->data, bias) ; 
#endif
    return result_matrix;
}
...
// HW FUNCTIONS
//------------------------------------
Matrix_uint8* ACCELERATOR_matrixFCNActivate(Matrix_uint8* input_matrix, Matrix_int8* weight_matrix, Matrix_int32* bias_vector, Matrix_uint8* result_matrix, Allocator_int32* al ) {///  regs addresses for mannix accelerator (inputs for your blocks)
...
//  offset from the start of the accelerotor memory.
    int result_offset = weight_matrix->pad_size + input_matrix->pad_size + bias_vector->size*4;
    int bias_offset = weight_matrix->pad_size + input_matrix->pad_size;
    int input_offset = weight_matrix->pad_size;
    int weight_offset = 0;
                         //volatile signed int * fc_weight = mannixDataMalloc_int32(al, m1->pad_size);

//  this part is  for access the memory. we need to load the data the accelerator will use
    volatile unsigned int * fc_result = (volatile unsigned int * )(0x1a480000 + result_offset);
 //   printf("\nfc_result(%x)\n",fc_result);
    volatile unsigned int * fc_bias   = (volatile unsigned int * )(0x1a480000 + bias_offset);  
 //   printf("\nfc_bias(%x)\n",fc_bias);
    volatile unsigned int * fc_input  = (volatile unsigned int * )(0x1a480000 + input_offset);
 //   printf("\nfc_input(%x\n)",fc_input);
    volatile signed int * fc_weight   = (volatile signed   int * )(0x1a480000 + weight_offset);
 //   printf("\nfc_wieght(%x)\n",fc_weight);

    int i;

// writing data to memory
// TODO move to the main and duplicate for regular mem
    for ( i=0; i <  bias_vector->pad_size; i++) {
	* (fc_bias + i) = bias_vector->data[i];
    }
   
    for ( i=0; i <  weight_matrix->pad_size; i = i + 4) {
	int a=0;
        a =     ( signed int )weight_matrix->data[i + 0];
        a = a | ( signed int )weight_matrix->data[i + 1] << 8*1;
        a = a | ( signed int )weight_matrix->data[i + 2] << 8*2;
        a = a | ( signed int )weight_matrix->data[i + 3] << 8*3;

	* (fc_weight + i/4) = a;
    }
 
   for ( i=0; i <  input_matrix->pad_size; i = i + 4) {
       int a=0;
       a =     ( unsigned int )input_matrix->data[i + 0];
       a = a | ( unsigned int )input_matrix->data[i + 1] << 8*1;
       a = a | ( unsigned int )input_matrix->data[i + 2] << 8*2;
       a = a | ( unsigned int )input_matrix->data[i + 3] << 8*3;
   ...
   printf("(%d)",result_matrix->rows); 
   int done;

   for (i=0; i <  result_matrix->pad_size/4 ; i++ ) {
	int a=0;
 	* (fc_result + i) = a;
   }
		
    * fc_addrx = input_offset;
    * fc_addry = weight_offset;
    * fc_addrz = result_offset;
    * fc_addrb = bias_offset;
...
//	printf("\r waiting(%d)",fc_done);	
 //  }	
   ...
   * fc_start = 1;
   ...
   printf("(%d)",result_matrix->rows); 
   for (int i=0; i <  result_matrix->pad_size/4; i++) {
     int a=0;
     int tmp = 0;
     a = * (fc_result + i);
     tmp = a;
     printf("\n\na = %x - %x \n\n",a,&* (fc_result + i));
     result_matrix->data[i*4 + 0] = (tmp & 0x000000ff) ;
     tmp = a;
     
     result_matrix->data[i*4 + 1] = ((tmp & 0x0000ff00) >> 8*1) ;
     tmp = a;
     result_matrix->data[i*4 + 2] = ((tmp & 0x00ff0000) >> 8*2) ;
     tmp = a;     
     result_matrix->data[i*4 + 3] = ((tmp & 0xff000000) >> 8*3) ;
...
int compareMatrices(Matrix_uint8 * m1, Matrix_uint8 * m2){
  if(m1->rows != m2->rows || m1->cols != m2->cols) {
    printf("\nERROR - CHECK MATRICES DIM\n");
  }
  int valid = 1;
  for (int i = 0; i < m1->size; i++) {
    if (m1->data[i] != m2->data[i]) valid = 0;
  }
 // if (valid) printf("success - matrices are equal\n");
 // else printf("\nfailed - different results\n\n");
  return valid;
}
//Matrix_uint8* ACCELERATOR_matrixMaxPool(Matrix_uint8* m1, Matrix_uint8* m2, int p_m, int p_n, int stride){
//}
