...
void setMatrixToTensor_uint8(Tensor_uint8* tens, FILE_PTR filePointer, unsigned int* label, int op) {
...
int compareTensors(Tensor_uint8* t1, Tensor_uint8* t2) {
	int valid = 1;
	for (int i = 0; i < t1->depth; i++) {
		if(!compareMatrices(&t1->matrix[i], &t2->matrix[i])) valid = 0;
	}
	return valid;
}
...
Tensor_uint8* tensorMaxPool_uint8(Tensor_uint8 *tens, Tensor_uint8 *tens_result, int p_m, int p_n, int stride,int work_mode){
    if(ACCELERATOR_MODE == work_mode)
    for(int d = 0; d < tens->depth; d++) {
        matrixMaxPool(&tens->matrix[d], &tens_result->matrix[d], p_m, p_n, stride);
    }
    else	
      for(int d = 0; d < tens->depth; d++) {
        ACCELERATOR_matrixMaxPool(&tens->matrix[d], &tens_result->matrix[d], p_m, p_n, stride);
      }
    return tens;
}
