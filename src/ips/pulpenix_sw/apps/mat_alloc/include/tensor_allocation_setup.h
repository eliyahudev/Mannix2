 ...
 #define TENSOR_ALLOCATION_SETUP
    ...
    unsigned char data[MANNIX_DATA_SIZE];  
    unsigned char alloc_matrix[MANNIX_MAT_SIZE*sizeof(char)];
    unsigned char tens[MANNIX_TEN_SIZE*sizeof(char)];
    ...
    //char vol_data[MANNIX_DATA_SIZE];
    volatile char* vol_data = GPP_BASE_MEM;
