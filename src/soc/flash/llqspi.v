		...
		if (i_rst) begin
			...
			if ((state == `QSPI_IDLE)&&(o_sck)) begin
				     ...
				     if ((i_wr)&&(!o_busy)) begin
				     	state <=#1 `QSPI_START;
				     	spi_len<=#1 { 1'b0, i_len, 3'b000 } + 6'h8;
				     	o_cs_n <=#1 1'b0;
				     	// o_sck <=#1 1'b1;
				     	o_busy <=#1 1'b1;
				     end
			end 
			else if (state == `QSPI_START) begin // We come in here with sck high, stay here 'til sck is low
				      o_sck <=   1'b0;
				      if (o_sck == 1'b0) begin
				      	state <=#1 `QSPI_BITS;
				      	spi_len<=#1 spi_len - ( (r_spd)? 6'h4 : 6'h1 );
				      	if (r_spd) r_word <=#1 { r_word[27:0], 4'h0 };
				      	else r_word <=#1 { r_word[30:0], 1'b0 };
				      end
				      o_mod <=#1 (r_spd) ? { 1'b1, r_dir } : `QSPI_MOD_SPI;
				      o_cs_n <=#1 1'b0;
				      o_busy <=#1 1'b1;
				      o_valid <=#1 1'b0;
				      if (r_spd) 	o_dat <=#1 r_word[31:28];
				      else		o_dat <=#1 { 3'b110, r_word[31] };
			end 
			else if (!o_sck) begin
				      o_sck <=   1'b1;
				      o_busy <=#1 ((state != `QSPI_READY)||(!i_wr));
				      o_valid <=#1 1'b0;
			end 
				      ...
				      o_busy <=#1 1'b1;
				      if (r_spd) begin
				      	o_dat <=#1 r_word[31:28];
				      	r_word <=#1 { r_word[27:0], 4'h0 };
				      	spi_len <=#1 spi_len - 6'h4;
				      	if (spi_len == 6'h4) state <=#1 `QSPI_READY;
				      end 
				      else begin
				      	o_dat <=#1 { 3'b110, r_word[31] };
				      	r_word <=#1 { r_word[30:0], 1'b0 };
				      	spi_len <=#1 spi_len - 6'h1;
				      	if (spi_len == 6'h1) state <=#1 `QSPI_READY;
				      end
				      o_valid <=#1 1'b0;
				      if (!o_mod[1]) r_input <=#1 { r_input[29:0], i_miso };
				      else if (o_mod[1]) r_input <=#1 { r_input[26:0], i_dat };
			end 
				      ...
				      r_dir <=#1 i_dir;
				      if (i_spd) begin
				      	r_word <=#1 { i_word[27:0], 4'h0 };
				      	spi_len<=#1 { 1'b0, i_len, 3'b000 } + 6'h8 - 6'h4;
				      end 
				      else begin
				      	r_word <=#1 { i_word[30:0], 1'b0 };
				      	spi_len<=#1 { 1'b0, i_len, 3'b000 } + 6'h8 - 6'h1;
				      end
				      if((!o_busy)&&(i_wr)) begin // Acknowledge a new request
				      	...
				      	o_mod <=#1 (i_spd) ? { 1'b1, i_dir } : `QSPI_MOD_SPI;
				      	if (i_spd) o_dat <=#1 i_word[31:28];
				      	else o_dat <=#1 { 3'b110, i_word[31] };
				      end 
				      else begin
				      	o_sck <=  1'b1;
				      	state <=#1 (i_hold)?`QSPI_HOLDING : `QSPI_STOP;
				      	o_busy <=#1 (!i_hold);
				      end
				      // Read a bit upon any transition
				      o_valid <=#1 1'b1;
				      if (!o_mod[1]) begin
				      	r_input <=#1 { r_input[29:0], i_miso };
				      	o_word  <=#1 { r_input[30:0], i_miso };
				      end 
				      else if (o_mod[1]) begin
				      	r_input <=#1 { r_input[26:0], i_dat };
				      	o_word  <=#1 { r_input[27:0], i_dat };
				      end
			end 
				      ...
				      if (i_spd) begin
				      	r_word <=#1 { i_word[27:0], 4'h0 };
				      	spi_len<=#1 { 1'b0, i_len, 3'b100 };
				      end 
				      else begin
				      	r_word <=#1 { i_word[30:0], 1'b0 };
				      	spi_len<=#1 { 1'b0, i_len, 3'b111 };
				      end
				      if((!o_busy)&&(i_wr)) begin// Acknowledge a new request
				      	state  <=#1 `QSPI_BITS;
				      	o_busy <=#1 1'b1;
				      ...
				      o_valid <=#1 1'b0; // Output may have just been valid, but no more
				      o_busy  <=#1 1'b1; // Still busy till port is clear
				      state <=#1 `QSPI_STOP_B;
				      o_mod <=#1 `QSPI_MOD_SPI;
			end 
				      ...
				      o_sck <=   1'b1;
				      // Do I need this????
				      // spi_len <=#1 3; // Minimum CS high time before next cmd
				      state <=#1 `QSPI_IDLE;
				      o_valid <=#1 1'b0;
				      o_busy <=#1 1'b1;
				      o_mod <=#1 `QSPI_MOD_SPI;
...
`ifdef	FORMAL
	...
	*/
	...
	always @(posedge i_clk)
		...
		begin
			if (o_mod == `QSPI_MOD_QOUT)
				f_shiftreg <=#1 { f_shiftreg[28:0], o_dat };
			else if (o_mod == `QSPI_MOD_SPI)
				f_shiftreg <=#1 { f_shiftreg[30:0], o_dat[0] };
		end
	...
	always @(posedge i_clk)
		...
		begin
			if (o_mod == `QSPI_MOD_SPI)
				f_nsent <=#1 f_nsent + 6'h1;
			else
				f_nsent <=#1 f_nsent + 6'h4;
		end
	always @(posedge i_clk)
		if (o_cs_n)
			f_vsent <=#1 0;
